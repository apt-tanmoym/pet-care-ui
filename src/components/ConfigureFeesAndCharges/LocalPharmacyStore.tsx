...
import {
  ...
  Select, MenuItem, IconButton, InputAdornment, TableContainer, Table, TableHead, TableRow, TableCell, Paper, TableBody, Button, TablePagination,
  Autocomplete,
  debounce
} from '@mui/material';
...
import { checkDuplicate, checkDuplicatePhermecyName, deleteProcedure, getCounsultationFees, getMedicineList, getPharmacyitemDetails, getProcedureList, removeMedicineList, saveMedicineList, saveProcedureList } from '@/services/feesAndCharges';
...
const LocalProcedure: React.FC = () => {
  ...
  const [errors, setErrors] = useState<{ procedureName?: string; price?: string, brandName?:string; }>({});
  ...
  const checkDuplicate = async(checkDulicateObj:any) => {
    ...
    if(result !== 'Pharmacy Item Not Found') {
      ...
      setSnackbarMessage("This name already exists please try with other name.")
      return false;
    }else {
      return true;
    }    
  ...
  const handleSubmit = async () => {
    ...
    if (!procedureName.trim()) newErrors.procedureName = 'Medicine name is required';
    if (!price || isNaN(+price) || +price <= 0) newErrors.price = 'Valid rate required';
    ...
    if(modalMode === 'add'){
      ...
      const checkDuplicateObj = {
        ...counsltationPayload,
        medicineName: selectedBrand?.medicineBrandName ? selectedBrand.medicineBrandName : brandName,
  ...
  return (
    <Container>
      <Typography variant="caption" color="textSecondary" mb={2} display="block">
        *Local Pharmecy fees will be collected at confirmation of encounter consultation data {paginatedRowData.length}
      </Typography>
      ...
      <Button onClick={openAddModal} variant="contained" color="primary" sx={{ ml: "auto" }}>
        Add New Pharmecy
      </Button>    
    </Box>
