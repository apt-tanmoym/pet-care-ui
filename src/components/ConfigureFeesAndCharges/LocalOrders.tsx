...
import {  checkDuplicateOrderName, deleteOrder, getCounsultationFees, getOrderList, saveOrderName } from '@/services/feesAndCharges';
...
const LocalOrder: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    if(modalMode === 'add'){
      ...
      const isDuplicate = await checkIsDuplicate(selectedOrder? selectedOrder.orderTestName : orderName)
      if(isDuplicate) {
      const addOrderObj = {
        ...counsltationPayload,
        orderId: 0,
  ...
  return (
    <Container>
      <Typography variant="caption" color="textSecondary" mb={2} display="block">
        *Local Orders will be collected at confirmation of encounter consultation data {paginatedRowData.length}
      </Typography>
