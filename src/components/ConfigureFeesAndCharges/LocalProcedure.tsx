...
import { checkDuplicate, deleteProcedure, getCounsultationFees, getProcedureList, saveProcedureList } from '@/services/feesAndCharges';
...
const LocalProcedure: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    if(modalMode === 'add'){
      ...
      console.log(price)
      const isDuplicate = await checkIsDuplicate(selectedProcedure? selectedProcedure.procedureName : procedureName)
      if(isDuplicate) {
      const addProcudureObj = {
        ...counsltationPayload,
        procedureId: 0,
        procedureName: selectedProcedure? selectedProcedure.procedureName : procedureName,
        charge: price
      }
  ...
  return (
    <Container>
      <Typography variant="caption" color="textSecondary" mb={2} display="block">
        *Local Procedure fees will be collected at confirmation of encounter consultation data {paginatedRowData.length}
      </Typography>
