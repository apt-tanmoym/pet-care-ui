...
import { Close as CloseIcon, Delete as DeleteIcon, Visibility as VisibilityIcon, Search as SearchIcon, DragIndicator as DragIndicatorIcon } from '@mui/icons-material';
...
const SortableItem: React.FC<SortableItemProps> = ({ allergy, index, onShowAllergy, onResolveAllergy, onRemoveAllergy }) => {
  const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: allergy.id });
  ...
  return (
    <Paper
      ...
      style={style}
      elevation={1}
      sx={{ p: 1.5, mb: 1.5, bgcolor: 'rgba(24, 40, 72, 0.05)', '&:hover': { bgcolor: 'rgba(24, 40, 72, 0.1)' } }}
    >
      <Stack spacing={0.5} alignItems="flex-start">
        ...
        <Typography variant="caption" color="text.secondary">
          Reported: {allergy.date} {allergy.time} | Recorded by: {allergy.recordedBy} |{' '}
          <Typography component="span" color={allergy.isActive ? 'error.main' : 'success.main'}>
            {allergy.isActive ? 'Active' : 'Resolved'}
          </Typography>
        </Typography>
        <Stack direction="row" spacing={0.5} sx={{ mt: 0.5 }}>
          <Tooltip title="View allergy details">
            <IconButton size="small" color="primary" onClick={() => onShowAllergy(allergy)}>
              <VisibilityIcon fontSize="small" />
            </IconButton>
          </Tooltip>
          {allergy.isActive && (
            <Tooltip title="Resolve this allergy">
              <IconButton size="small" color="secondary" onClick={() => onResolveAllergy(index)}>
                <CloseIcon fontSize="small" />
              </IconButton>
            </Tooltip>
          )}
          <Tooltip title="Remove this allergy">
            <IconButton size="small" color="error" onClick={() => onRemoveAllergy(index)}>
...
const Allergy: React.FC<AllergyProps> = ({ record }) => {
  ...
  const [allergiesHistory, setAllergiesHistory] = useState<AllergyEntry[]>(
    Array.isArray(record.allergies) && record.allergies.length > 0
      ...
      : [
          { date: '26/08/2025', time: '09:00 AM', description: 'Penicillin', severity: 'Severe', isActive: true, recordedBy: 'Dr. Smith', id: 'allergy-0' },
          { date: '25/08/2025', time: '02:30 PM', description: 'Pollen', severity: 'Mild', isActive: false, recordedBy: 'Dr. Jones', id: 'allergy-1' },
          { date: '24/08/2025', time: '11:15 AM', description: 'Peanuts', severity: 'Moderate', isActive: true, recordedBy: 'Dr. Lee', id: 'allergy-2' },
        ]
  );
    ...
    <Box sx={{ p: { xs: 2, sm: 3, md: 3 }, bgcolor: 'grey.50', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <Paper elevation={1} sx={{ p: 2, mb: 2, background: 'linear-gradient(135deg, #4B6CB7 0%, #182848 100%)', color: 'white' }}>
        ...
        <Typography variant="subtitle1" color="rgba(255, 255, 255, 0.9)">
          Detailed view of patient&apos;s allergies for {record.patientInfo.patientName}
        </Typography>
      </Paper>
                  ...
                  <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                    <SortableContext items={filteredAllergies.map((a) => a.id)}>
                      {filteredAllergies.length > 0 ? (
                        ...
                        <Typography variant="body2" color="text.secondary" sx={{ p: 2, textAlign: 'center', fontSize: '0.9rem' }}>
          ...
          {selectedAllergy && (
            <Stack spacing={1}>
              <Typography variant="body1" color="text.primary" sx={{ fontSize: '0.9rem' }}>
                {selectedAllergy.description} (Severity: {selectedAllergy.severity})
              </Typography>
              <Typography variant="caption" color="text.secondary">
              ...
              <Typography variant="caption" color={selectedAllergy.isActive ? 'error.main' : 'success.main'}>
