...
const ManageUsersEdit: React.FC<ManageUsersEditProps> = ({ user, onSubmit, onCancel, roleGroupList }) => {
  ...
  const validationRules: { [key in keyof (User & { image: string })]?: (value: any, preview?: string | null) => string } = {
    ...
    image: (file: File | null, preview?: string | null | undefined) => (file || preview ? '' : 'Image is required'),
  };
  ...
  useEffect(() => {
    if (selectedCity) {
     ...
     const areas: any =  fetchAreas(String(user.cityId),'')
      ...
      setErrors((prev) => ({ ...prev, city: '', state: '', country: '', areaName: '' }));
    }
  }, [selectedCity]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append("userPwd", "P@ssw0rd"); // 🔹 Do you want to hardcode or capture?
      formData.append("orgId", "45");         // 🔹 Probably from context/auth?
          ...
          <Autocomplete
            ...
            onChange={(_, value) => {
              if (value && typeof value !== 'string') {
                ...
                setFormValues((prev) => ({ ...prev, city: value || '', state: '', country: '', areaName: '' }));
                setAreaOptions([]);
              }
              setApiError('');
            }}
          ...
          <Autocomplete
            ...
            onChange={(_, value) => {
              if (typeof value === 'string') {
                ...
                setErrors((prev) => ({ ...prev, areaName: validationRules.areaName!(value) }));
              } else if (value && value.areaName) {
                setAreaMappingId(value?.cityPincodeMappingId || null);
                setFormValues((prev) => ({ ...prev, areaName: value.areaName }));
