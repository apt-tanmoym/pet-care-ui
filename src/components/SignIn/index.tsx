...
import { login, getOrganizationList, getLocationsDropdown } from '@/services/loginService';
...
const SignIn: React.FC = () => {
  ...
  const fetchFacilities = useCallback(async (userName: string, orgId: number) => {
    ...
    try {
      const facilities = await getLocationsDropdown(userName, orgId.toString());
      ...
      if (mappedFacilities.length > 0) {
        setValue('orgFacilityId', mappedFacilities[0].id, { shouldValidate: true });
      }
    } catch (error: any) {
      setSnackbarMessage(error?.response?.data?.message || 'Failed to load facilities');
  ...
  const fetchOrganizations = useCallback(async (userName: string) => {
    ...
    try {
      ...
      setSnackbarMessage(error?.response?.data?.message || 'Failed to load organizations');
      setSnackbarSeverity('error');
      setOpenSnackbar(true);
      setOrganizations([]);
      setValue('orgId', 0);
    ...
    <Container className={styles.formContainer}>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
            <FormControl fullWidth error={!!errors.orgId} sx={{ '& .MuiFormHelperText-root': { marginTop: 0, minHeight: '20px' } }}>
              <InputLabel>Organization</InputLabel>
              <Select
                {...register('orgId', { required: 'Organization is required' })}
                label="Organization"
                value={selectedOrgId}
                onChange={e => setValue('orgId', Number(e.target.value), { shouldValidate: true })}
                className={styles.formField}
          ...
          <Grid item xs={12}>
            <FormControl fullWidth error={!!errors.orgFacilityId} sx={{ '& .MuiFormHelperText-root': { marginTop: 0, minHeight: '20px' } }}>
              <InputLabel>Facility</InputLabel>
              <Select
                {...register('orgFacilityId', { required: 'Facility is required' })}
                label="Facility"
                value={watch('orgFacilityId')}
                onChange={e => setValue('orgFacilityId', Number(e.target.value), { shouldValidate: true })}
                className={styles.formField}
              ...
              <FormHelperText>{errors.orgFacilityId?.message || ' '}</FormHelperText>
            </FormControl>
          </Grid>
          <Grid item xs={12}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <FormControlLabel
                ...
                label={<Typography variant="body2" className={styles.checkboxLabel}>Remember me</Typography>}
              />
              <Button
