...
import {
  Box, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Avatar, Typography, TablePagination, Grid, TextField, Select, MenuItem
} from '@mui/material';
...
const DoctorsTable: React.FC = () => {
  ...
  const paginatedDoctors = dummyDoctors.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
        ...
        <Grid container spacing={2} alignItems="center" mb={2}>
          ...
          <Grid item xs={12} md={3} display="flex" justifyContent="flex-end">
            <Button
              variant="contained"
              sx={{ bgcolor: '#174a7c' }}
              onClick={() => { setDetailsOpen(true); setSelfConfirmOpen(true); }}
            >
              Add New Doctor
            </Button>
          </Grid>
              ...
              {paginatedDoctors.length === 0 ? (
                ...
                paginatedDoctors.map((doc) => (
                  <TableRow key={doc.id}>
                    ...
                    <TableCell>
                      <Button size="small" variant="outlined" color="secondary" startIcon={<EditIcon />} onClick={() => { setEditDoctorData(doc); setEditModalOpen(true); }}>Edit</Button>
                    </TableCell>
                    <TableCell>
                      <Button size="small" variant="outlined" onClick={() => { setPrivilegeDoctor(doc); setPrivilegeModalOpen(true); }}>Assign User Privilege</Button>
                    </TableCell>
                  </TableRow>
                ))
              )}
        ...
        {privilegeModalOpen && privilegeDoctor && (
          <AssignUserPrivilegeModal
            ...
            user={{
              ...
              lastName: privilegeDoctor.name.split(' ').slice(1).join(' ') || '',
